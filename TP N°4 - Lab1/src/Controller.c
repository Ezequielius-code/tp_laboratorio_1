...
int controller_addEmployee(LinkedList* pArrayListEmployee)
{
	...
	if (nuevoEmp != NULL)
	{
		utn_GetName(auxNombre,"\nIngrese nombre del empleado: ",128);
		auxHorasTrabajadas = utn_getNumero("\nIngrese cantidad de horas trabajadas (MAX 325): ","\nERROR. Cantidad de horas no validas.",1,325);
		auxSueldo = utn_getNumero("\nIngrese sueldo del empleado: ","\nERROR. Sueldo no valido.",10000,50000);

		if(auxNombre != NULL && auxHorasTrabajadas != 0 && auxSueldo != 0)
		{
			employee_setId(nuevoEmp,ultimoId);
	...
	if (aux != NULL)
	{
		do
		{
			...
			switch (opcion)
			{
				...
				break;
				case 2:
					auxHorasTrabajadas = utn_getNumero("\nIngrese nueva cantidad de horas trabajadas: ","\nERROR. La cantidad de horas no es valida.",1,325);
...
int controller_sortEmployee(LinkedList* pArrayListEmployee)
{
	...
	do
	{
		...
		printf("\n9.Salir");
		opcion = utn_getNumero("\n\n*Seleccione la opcion deseada: ","\nERROR. Ha ingresado una opcion no valida.",1,9);
...
int controller_raiseSalary(LinkedList* pArrayListEmployee)
{
	int retorno = -1;
	LinkedList* nuevoSueldo = NULL;

	if(pArrayListEmployee != NULL)
	{
		nuevoSueldo = ll_map(pArrayListEmployee,aumentarSueldos);
		retorno = 0;
	}

	controller_saveAsText("dataAumento.csv",nuevoSueldo);
	ll_deleteLinkedList(nuevoSueldo);
